#!/usr/bin/env python3
"""
depth_num_sorting_node.py — 深度增强数字分拣

使用 Aurora 930 深度相机 + TFLite 数字检测 + 世界系定位。
检测逻辑参考 num_sorting_node.py，坐标定位参考 depth_color_sorting_node.py。
"""
...
class DepthNumSortingNode(Node):
    def __init__(self):
        super().__init__('depth_num_sorting_node')
        ...
        self.pid_x = PIDController(kp=0.005, ki=0.000, kd=0.0)
        self.pid_y = PIDController(kp=0.005, ki=0.000, kd=0.0)

        self.debug_pub = self.create_publisher(Image, '/depth_num_sorting/image_result', 10)
        ...
        # enter/exit 服务
        self.enter_srv = self.create_service(Trigger, '/depth_num_sorting/enter', self.enter_callback)
        self.exit_srv = self.create_service(Trigger, '/depth_num_sorting/exit', self.exit_callback)

        self.get_logger().info('\033[1;36m[DepthNumSort]\033[0m 深度增强数字分拣已启动')
    ...
    def _load_model(self):
        ...
        self.get_logger().info(f'[模型] 输入:{input_det["shape"]} 输出:{output_det["shape"]} 类别:{len(labels)}')
    ...
    def run(self):
        ...
        self.move_status = 0
        self.current_num_index = 0
        self.get_logger().info('\033[1;32m[DepthNumSort]\033[0m 分拣启动')
        ...
        while self.sorting_active and rclpy.ok():
            ...
            if fc % 150 == 0 or self._last_logged_status != self.move_status:
                self.get_logger().info(
                    f'[NUM] f={fc} target={target_num} found={found} st={self.move_status}')
                self._last_logged_status = self.move_status

            cv2.putText(frame, f"Target: {target_num}", (10, 25),
    ...
    def _st0(self):
        ...
        self.move_status = 1
        self.get_logger().info(f'[st0] num={self.num} XYZ={self.world_target_mm}')
    ...
    def _st5(self):
        # 数字分拣区位置（参考 num_sorting_node.py）
        if self.num == '1':
            self.move_x, self.move_y = 145, 60
        elif self.num == '2':
    ...
    def _st6(self):
        ...
        if not self.color_read_succed:
            self._st6_miss_cnt = getattr(self, '_st6_miss_cnt', 0) + 1
            if self._st6_miss_cnt >= 30:  # ~1秒未检测到，强制放下
                self.get_logger().warn('[st6] 检测超时，强制放置')
                self._st6_miss_cnt = 0
                self.move_status = 7
            return
        self._st6_miss_cnt = 0
    ...
    def _st7(self):
        ...
        if l > 0:
            ...
            self.move_x = int((l + 56) * c)
            self.move_y = int((l + 56) * s)
        kinematics_move(self.move_x, self.move_y, 60, 1000)
        time.sleep(1)
    ...
    def _st9(self):
        ...
        if self.current_num_index >= len(self.target_nums):
            self.current_num_index = 0
            self.get_logger().info('[完成] 所有数字分拣完成，重置')
        self.color_read_succed = 0
        self.move_status = 0
        self.world_target_mm = None
    ...
    def enter_callback(self, request, response):
        self.get_logger().info('收到Enter服务，启动深度数字分拣！')
        if not self.active:
            try:
        ...
        response.message = '深度数字分拣已启动'
        return response

    def exit_callback(self, request, response):
        self.get_logger().info('收到Exit服务，停止深度数字分拣！')
        if self.active:
            self.active = False
            self.sorting_active = False
            close_uart()
        ...
        response.success = True
        response.message = '深度数字分拣已停止'
        return response

def main(args=None):
    rclpy.init(args=args)
    node = DepthNumSortingNode()
