...
class YoloDetectNode(Node):
    ...
    def detect(self):
        ...
        for det in detections:
            ...
            conf = det['confidence']
            area = det['area']
            area_pct = area / total_area * 100  # 面积占比(%)

            # 边界框
            cv2.rectangle(annotated, (x1, y1), (x2, y2), (0, 255, 0), 2)

            # 标签文字：类别 置信度 面积 面积占比
            text = f"{label} {conf:.2f} A:{area}px ({area_pct:.1f}%)"
        ...
        # 顶部信息栏
        info = f"Detected: {len(detections)} | Frame: {w_img}x{h_img}"
    ...
    try:
        while rclpy.ok():
            ...
            if skip_counter % skip_frames == 0:
                last_dets = node.detect()
            skip_counter += 1
            frame_count += 1
            if frame_count % 30 == 0:
                node.get_logger().info(f'已处理 {frame_count} 帧(推理{frame_count//skip_frames}次)，检测到 {len(last_dets)} 个物体')
            time.sleep(0.05)
